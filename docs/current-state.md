...
Last updated: 2026-07-19 (Claude Code — reviewed Antigravity's Phase 3/4 work, fixed one regression, applied two user-requested follow-ups, and merged the shared-item-queue-into-add-product change; full details below).
...
5. **Branch-topology fix: moved these two fix commits off the unauthorized Phase 4 branch.** The bulk-cool-duration-regression fix (`af8a94c`) and the share-queue-merge commit had both ended up committed directly onto `phase-4/shared-ghost-attribution-notifications` — the branch Antigravity has been using for its **not-yet-approved** Phase 4 (shared-ghost-attribution/notifications) work, which still has uncommitted in-progress changes sitting in that branch's working tree. Since neither fix is Phase 4 work, they were cherry-picked (via a temporary `git worktree`, so Antigravity's uncommitted files were never touched) onto `phase-3/share-queue-location-animation`, where they now live cleanly as `d289b32` and `67e9321`. **`phase-4/shared-ghost-attribution-notifications` was left completely untouched** (still has its own copies of these two commits plus Antigravity's uncommitted Phase 4 files on top) — nobody should rebase or force-push that branch without knowing this. The authoritative, isolated copy of the bug-fix + share-queue-merge work is on `phase-3/share-queue-location-animation` at `67e9321`.

### Repository and release state

- This branch, `phase-3/share-queue-location-animation` (head `67e9321`), is the authoritative, isolated home for Phase 3 plus the two follow-up fixes (bulk-cool-duration regression, share-queue-into-add-product merge). `phase-4/shared-ghost-attribution-notifications` branches off this same tip but also carries Antigravity's separate, **not-yet-approved** Phase 4 work plus uncommitted in-progress files — see branch-topology note above before touching that branch.
- Latest product implementation: current head of this branch (`67e9321` — v2.7.12 Phase 3 multi-share queue/location simulation, plus the two follow-up fixes above).
