...
## 2026-07-25 — Claude Code (explicit user override: reverted primary action to cart-first, kept Orders/grouping)

- **Reverted the primary catalogue/product-page trigger from Codex's "Ghost it starts cooldown immediately" back to "Add to cart" with no cooldown until checkout — this is a deliberate, explicit user product decision, not a bug fix or a misunderstanding of the Codex handoff.** The user confirmed cart-first was their originally intended model; the instant-cooldown version from the same day's earlier Codex session was not what they wanted. **Why override rather than adjust:** the user stated cart-first directly, twice, as the correct behavior for this product - the CLAUDE CODE HANDOFF section above (dated the same day) documents the instant-cooldown model as current, but that model is superseded by this entry for the primary catalogue/product-page action specifically. If the two sections conflict, this entry wins for that one behavior; everything else Codex built (Orders grouping, `order_group_id`/`ghostOrderId`, the four-way per-item resolution, canonical Ghost-count-at-cooldown-start, the Food & delivery lane) is unchanged and still current.
- **Scope of the revert, precisely:** only the catalogue/product-page primary action changed (Home product cards, `CategoryBrowseScreen` cards, `ProductDetailScreen`'s main CTA) - all now call `addToCart()` (already existed, unchanged) instead of starting a cooldown, and none of them navigate anywhere after tapping; the existing toast ("Added _ to Ghost Cart") is the confirmation. The manual-capture/link-import screen's "Ghost it" submit button (`CaptureAlmostBuyScreen`) was deliberately left unchanged - capturing a specific almost-buy from elsewhere isn't a "browse then decide" moment the way a catalogue card is, and the user's instructions scoped this to "the primary product-page/catalogue button" specifically.
- **Checkout already did everything needed for group cooldown + delivery simulation - zero backend/business-logic changes required.** `AppViewModel.placeSimulatedOrder()` already generates one `orderGroupId` and loops `createAlmostBuy()` for every item in the cart with that shared id (starting all their cooldowns together), and already calls `scheduleDeliveryWorkers()`/`beginDeliveryClock()` for the fake delivery ritual - this was pre-existing, unrelated to today's Codex changes, and simply needed to be reachable again. The only real gap was that `GhostCartList` (the cart screen) had no entry point in the current navigation after Codex's rework - added a cart icon with an item-count badge to the Home top bar, next to the notifications bell.
- **Deleted `AppViewModel.quickGhostCatalogProduct()` and `prepareCatalogProduct()`** rather than leaving them as unused dead code - both existed solely to serve the two catalogue call sites just changed to `addToCart()`, had no other callers, and had no reason to survive the revert.
- **Reapplied the color/price Amazon-scraping fix and R2 image re-hosting** (`lib/product-link-preview.ts`, `lib/product-image-rehost.ts`, `app/api/product-images/[key]/route.ts`, `app/api/almost-buys/route.ts`) from `backup/color-price-r2-fix-81a5d9f`, a commit that had been made before Codex's Orders rework landed and was at risk of being lost. Reconciled cleanly via `git cherry-pick` - Codex's `order_group_id` addition and this fix's `storedImageUrl` swap sit in the same `INSERT INTO almost_buys` statement but touch different values, and git's 3-way merge combined them correctly with no manual edits needed (verified by reading the merged result, not just trusting a clean cherry-pick).
- **Reordered Home:** Marketplace products now render above Food & delivery (was the reverse). Removed three stale "using Ghost +" copy instances from `ProductDiscovery.kt` - the app has no button actually labeled "Ghost +" (the real central nav button is labeled "Ghost"), and one of the three claimed a share-from-food-app flow that doesn't work in practice (none of Talabat/Deliveroo/etc. actually offer Ghost Cart as a share target).
- **Seeded 10 curated demo products into the live `products` table for the Food & delivery lane** (`merchant_id = 1`, "Ghost Cart Demo Catalog"; category `"Food & delivery"`; ghost-themed names - Ghost Sushi Platter, Ghost Double Cheeseburger, Ghost Chocolate Donut, Ghost Shawarma Wrap, Ghost Alfredo Pasta, Ghost Iced Caramel, Ghost Fries, Ghost Fried Chicken, Ghost Pepperoni Pizza, Ghost Sundae; ids 39-48). User-supplied photos were validated (real PNG magic bytes, size/dimension caps) and metadata-stripped using the exact same `stripImageMetadata()` logic `lib/image-processing.ts` already uses for admin uploads, then uploaded to the live `ghostcart-content-media` R2 bucket and referenced via the existing `GET /api/content-blocks/image/[key]` route - the same end shape `POST /api/products/[id]/image` would have produced, just invoked via `wrangler r2 object put` + a direct `wrangler d1 execute --remote` INSERT instead of the authenticated HTTP admin API, since no admin session/credentials were available in this environment. **Both actions were explicit, user-confirmed writes to production** (the D1 insert specifically required an explicit go-ahead after an auto-mode classifier first blocked it) - not something to repeat casually for future seed data without the same confirmation.
