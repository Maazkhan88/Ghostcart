...
// Public OAuth client identifier generated by Firebase for the iOS app. The
// existing Worker secret remains the Android/web audience; accepting both
// preserves Android sign-in while iOS uses the server client from the same
// Firebase/Google Cloud project as its native client.
const IOS_WEB_CLIENT_ID = "172577078460-g55fm81e48qn3n3j4ko3daj7b2i23uej.apps.googleusercontent.com";
...
export async function POST(request: Request) {
  try {
    ...
    const verified = await verifyGoogleIdToken(idToken, [expectedAudience, IOS_WEB_CLIENT_ID]);
    if (!verified) {
      return Response.json(
