...
export async function POST(request: Request) {
  try {
    ...
    if (!isGiftToken(token)) return noStore({ error: "This Ghost Gift link is invalid" }, 400);
    if (payload.acceptedPrivacy !== true) {
      return noStore({ error: "Accept the privacy notice to reveal this Ghost Gift" }, 400);
    }
    ...
    const row = await db.prepare(
      ...
      id: string; status: string; expiresAt: string; title: string;
      category: string; imageUrl: string | null; amountCents: number;
      senderName: string | null;
    }>();
    if (!row) return noStore({ error: "This Ghost Gift is no longer available" }, 404);
    if (row.status === "withdrawn" || row.status === "reported") {
      return noStore({ error: "This Ghost Gift is no longer available" }, 410);
    }
    if (row.status === "expired" || Date.parse(row.expiresAt) <= Date.now()) {
      ...
      return noStore({ error: "This Ghost Gift link has expired" }, 410);
    }
    ...
    return noStore({ error: "Unable to reveal this Ghost Gift right now" }, 500);
  }
}
